 ...
 * <p>
 * Example: [source,java] -- // make sure everything sent to System.err is logged System.setErr(new
 * PrintStream(new LoggingOutputStream(Logger.getRootCategory()), true));
 * <p>
 * // make sure everything sent to System.out is also logged System.setOut(new PrintStream(new
 * LoggingOutputStream(Logger.getRootCategory()), true)); --
 *
...
public class Slf4jOutputStream extends OutputStream {
   ...
   * general contract of `close` is that it closes the output stream. A closed stream cannot perform
   ...
   * contract of `flush` is that calling it is an indication that, if any bytes previously written
   * have been buffered by the implementation of the output stream, such bytes should immediately be
